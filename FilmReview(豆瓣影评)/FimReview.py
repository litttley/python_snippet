...
def searchData(start,num,count):
    #设置请求头，
    ...
    try:
        ...
        req = requests.get(url='https://movie.douban.com/subject/24773958/comments?start='+str(start)+'&limit=20&sort=new_score&status=P&comments_only=1',headers=headers)  # 最基本的GET请求
        if req.status_code == 200:
            ...
            for i in pflags :#遍历列表,获取每个p标签的内容，即用户评论
                ...
                if (i.string != '') and (i.string is not None):
                    ...
                    str2 = "INSERT INTO comment VALUES('{0}', '{1}');\n".format(num,sqlStr)
                    with open("./comment.sql", encoding="utf-8", mode="a") as data:
                        data.write(str2)
                    num=num+1
                    if num >count:
                        return num
                    print("评论(%s):%s已成功写入#" % (num, sqlStr))
    ...
    while True :
            ...
            while True:
                if num > count:
                    ...
                    if name == "Y":
                        num=searchData(start * 20,num,count)  # 转换为url中的start值
                        start=start+1
                    else:
                        break
